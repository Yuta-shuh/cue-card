    ...
    <div className={`min-h-[100dvh] flex flex-col font-sans transition-colors duration-200 ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <header className={`p-4 md:p-6 shadow-sm flex flex-col gap-4 border-b ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        ...
        <textarea
          ...
          className={`w-full p-4 border rounded-xl resize-y h-32 md:h-40 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-medium text-lg ${
            isDark ? 'bg-gray-900 border-gray-600 text-white placeholder-gray-500 shadow-inner' : 'bg-gray-50 border-gray-300 text-black placeholder-gray-400 shadow-inner'
          }`}
        />
      </header>
      ...
      <main 
        ref={previewRef} 
        className={`flex-1 p-6 md:p-12 flex items-center justify-center overflow-auto selection:bg-blue-300 selection:text-blue-900 ${
